...
const AIAgent = ({ sprintCount, distractionCount, tasksCompleted, plannedSprints, meetingMinutes }) => {
  ...
  const messagePools = {
    gentle: {
      motivational: [
        ...
        "Nice work so far. You're building momentum.",
      ...
      tactical: [
        ...
        "Consider tackling that urgent task next.",
        "Your deep work items are waiting.",
  ...
  const generateMessage = () => {
    ...
    if (toughnessLevel === 1) {
      // Gentle mode
      pool = messagePools.gentle;
      if (sprintCount >= expectedSprints || expectedSprints === 0) {
        category = pool.motivational;
      } else {
        category = pool.tactical;
      }
    } else {
      // Tough love mode
    ...
    if (meetingHours > 2) {
      if (toughnessLevel === 1) {
        setMessage(`You've had ${meetingHours.toFixed(1)}h of meetings today. That's ${(meetingHours / hoursIntoWorkday * 100).toFixed(0)}% of your day. Time to focus on execution.`);
      } else {
    ...
    <div className="card ai-agent">
      <div className="card-header">
        ...
        >
          ...
          <option value={2}>🔥 Tough Love</option>
        </select>
      </div>
