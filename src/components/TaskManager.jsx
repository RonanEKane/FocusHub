...
const TaskManager = ({ onTaskComplete, onPlannedSprintsChange }) => {
  ...
  const [newTaskPriority, setNewTaskPriority] = useState('holding');
  ...
  const addTask = () => {
    ...
    setTasks(prev => ({
      ...prev,
      [newTaskPriority]: [...prev[newTaskPriority], task]
    }));
    
    setNewTask('');
    setNewTaskPriority('holding'); // Reset to holding
  };
  ...
  const plannedSprints = calculatePlannedSprints();
  const suggestedSprints = Math.ceil(plannedSprints * 1.3); // ADHD users underestimate by ~30%
    ...
    <div className="task-manager card">
      <div className="task-header">
        ...
        <div className="sprint-target">
          ...
          {plannedSprints > 0 && (
            <span className="sprint-hint" title="ADHD brains typically underestimate time by 30%">
              (AI suggests: {suggestedSprints})
            </span>
          )}
        </div>
      </div>
      ...
      >
        ...
        <div className="input-row">
          ...
          >
            ...
            <option value="urgent">🔥 Urgent (1 sprint)</option>
            <option value="deepwork">🎯 Deep Work (2 sprints)</option>
            <option value="strategic">♟️ Strategic (3 sprints)</option>
          </select>
          <input
            type="text"
            value={newTask}
