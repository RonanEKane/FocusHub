...
const EndOfDayModal = ({ sprintCount, plannedSprints, distractionCount, tasksCompleted, breakMinutes, meetingMinutes, onClose }) => {
  ...
  const handleSave = () => {
    ...
    // FIX: Move unfinished tasks back to holding, keep distractions
    const tasks = JSON.parse(localStorage.getItem('focushub_tasks') || '{}');
    
    // Tasks stay in their buckets (not deleted)
    // Distractions stay (not deleted) - user decides later
    ...
    <div className="modal-overlay">
      <div className="modal end-of-day-modal">
        <div className="modal-header">
          <h2 className="modal-title">📊 End of Day Report</h2>
        </div>
