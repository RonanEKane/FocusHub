...
const EndOfDayModal = ({ sprintCount, plannedSprints, distractionCount, tasksCompleted, breakMinutes, meetingMinutes, onClose }) => {
  ...
  useEffect(() => {
    ...
    setAutoGrade(calculated);
    setUserGrade(calculated); // Start with auto grade
  }, []);

  const allGrades = ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'D-', 'F'];
        ...
        <div className="modal-body">
          ...
          <div className="grade-display">
            <div className="grade-section">
              <div className="grade-label">System Grade</div>
              <div className={`grade-value grade-${autoGrade}`}>{autoGrade}</div>
            </div>
            
            <div className="grade-section">
              <div className="grade-label">Your Grade</div>
              <div className={`grade-value grade-${userGrade}`}>{userGrade}</div>
              <div className="grade-selector">
                {allGrades.map(grade => (
                  <button
                    key={grade}
                    className={`grade-btn ${userGrade === grade ? 'selected' : ''}`}
