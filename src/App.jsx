    ...
    <div className="app">
      <header className={`app-header ${headerScrolled ? 'scrolled' : ''}`}>
        <div className="logo-section">
          ...
          {!headerScrolled && <p className="tagline">Built for Brains That Wander, but Still Want to Win</p>}
          
          {/* Compact timer display when scrolled */}
          {headerScrolled && currentTimerState.isRunning && (
            <div className="header-timer-compact">
              <span className="timer-icon">{currentTimerState.timerType === 'focus' ? '🎯' : '☕'}</span>
              <span className="timer-value">
                {Math.floor(currentTimerState.timeLeft / 60)}:{(currentTimerState.timeLeft % 60).toString().padStart(2, '0')}
              </span>
            </div>
          )}
        </div>
