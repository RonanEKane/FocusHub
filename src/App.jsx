...
import { markAsReturningUser } from './utils/cookies';
...
function App() {
  ...
  const [taskSprints, setTaskSprints] = useState(0);
  ...
  const handlePlannedSprintsChange = (newTaskSprints) => {
    setTaskSprints(newTaskSprints);
    setPlannedSprints(baselineSprints + newTaskSprints);
  };
  ...
  const handleStartDay = (baseline, energy) => {
    try {
      console.log('Starting day with baseline:', baseline, 'energy:', energy);
      setBaselineSprints(baseline);
      setPlannedSprints(baseline); // Start with baseline, tasks will add to it
  ...
  if (dayEnded) {
    ...
    const handleStartNewDay = () => {
      ...
      localStorage.removeItem('focushub_session_date');
      localStorage.removeItem('focushub_daily_stats');
      localStorage.removeItem('focushub_tasks');
      localStorage.removeItem('focushub_completed_tasks');
    ...
    return (
      <div className="day-ended-screen">
        <div className="day-ended-content">
          ...
          <button onClick={handleStartNewDay} className="btn btn-primary">
            Start New Day
          </button>
        </div>
      </div>
